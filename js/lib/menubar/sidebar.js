...
!function ($) {
  ...
  sidebar.prototype.togglesidebar = function (type) {
    ...
    if (show) {
      _this.showsidebar();
      // Update the icon when the sidebar is open
      iconElement.removeClass('ti-angle-left').addClass('ti-angle-right');
    } else {
      _this.hidesidebar();
      // Update the icon when the sidebar is closed
      iconElement.removeClass('ti-angle-right').addClass('ti-angle-left');
    }
  }
